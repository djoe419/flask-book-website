...
from library.domain.model import Publisher, Author, Book, Review, User, BooksInventory, Shelve, ReadingList
...
class TestReview:
    ...
    def test_bind_book_object(self):
        book1 = Book(2675376, "Harry Potter")
        book2 = Book(874658, "Lord of the Rings")
        assert book1.reviews == []
        review1 = Review(book1, "I liked this book", 4)
        review2 = Review(book2, "This book was ok", 3)
        review3 = Review(book1, "This book was exceptional", 5)
        ...
        assert book1.reviews == [review1, review3]
        assert book2.reviews == [review2]
        with pytest.raises(ValueError):
...
class TestBooksJSONReader:
    ...
    def test_read_books_from_file_and_check_other_attributes(self, read_books_and_authors):
        ...
        assert dataset_of_books[0].num_pages is 0
        assert dataset_of_books[2].num_pages == 146
        assert dataset_of_books[5].num_pages == 206
        assert dataset_of_books[5].img_url == 'https://s.gr-assets.com/assets/nophoto/book/111x148-bcc042a9c91a29c1d680899eff700a03.png'
        assert dataset_of_books[5].website_url == 'https://www.goodreads.com/book/show/2250580.A_I_Revolution_Vol_1'
...
class TestBooksInventory:
    ...
    def test_construction_and_find(self):
        ...
        assert inventory.get_number_of_entries() == 2
    ...
    def test_remove_books(self):
        ...
        assert inventory.get_number_of_entries() == 0
        ...
        assert inventory.get_number_of_entries() == 1
...
class TestShelves:
    ...
    def test_add_reading_lists(self):
        shelve = Shelve()
        shelve.post_init()
        list1 = ReadingList("hello", shelve)

        shelve.add_reading_list(list1)
        print(shelve.reading_lists)
        assert list1 in shelve.reading_lists

    def test_remove_reading_list(self):
        shelve = Shelve()
        shelve.post_init()
        list1 = ReadingList("hello", shelve)

        shelve.add_reading_list(list1)
        shelve.remove_reading_list(list1)
        assert list1 not in shelve.reading_lists
